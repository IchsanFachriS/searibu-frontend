...
const ISLANDS: Island[] = [
  ...
  { id: "pramuka",     name: "Pulau Pramuka",     nameEn: "Pramuka Island",     lat: -5.745159, lon: 106.613782, adminZone: "Kepulauan Seribu Utara",   descId: "Pusat administrasi Kabupaten Kepulauan Seribu dengan fasilitas lengkap dan dermaga aktif.", descEn: "Administrative centre of Kepulauan Seribu with complete facilities and an active pier.", facilities: ["Resort Bintang 4","Kolam Renang","Tennis","Diving","Spa"] },
...
}> = ({ language, onIslandSelect, onCoordinateSearch }) => {
...
const LegendPanel: React.FC<{ language: string; defaultCollapsed?: boolean }> = ({ language, defaultCollapsed = false }) => {
  ...
  const items = [
    ...
    {
      icon: <div style={{ width: 10, height: 10, borderRadius: 2, background: "rgba(59,130,246,0.25)", border: "1.5px solid #3b82f6" }} />,
      label: language === "id" ? "Grid TPXO" : "TPXO Grid",
    },
  ];

  return (
    <div style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(12px)", borderRadius: 10, boxShadow: "0 4px 16px rgba(0,0,0,0.11)", border: "1px solid rgba(0,0,0,0.08)", overflow: "hidden", width: collapsed ? "auto" : 148, minWidth: collapsed ? 0 : 148 }}>
      <button
...
export const MapContainer: React.FC<MapContainerProps> = ({ basemap, onGridClick, onCoordinateSearch }) => {
  ...
  useEffect(() => {
    fetch("/GRID_TPXO_SERIBU.geojson").then((r) => r.json()).then(setGeojsonData).catch(console.error);
  }, []);
  ...
  const BASEMAP_HEIGHT = 40;
  const BASEMAP_BOTTOM = 16;
  const LEGEND_BOTTOM  = BASEMAP_BOTTOM + BASEMAP_HEIGHT + 8;
  ...
  return (
    <>
      ...
      <div
        ...
        onTouchMove={(e) => e.stopPropagation()}
      >
        <BottomSearchBar language={language} onIslandSelect={flyToIsland} onCoordinateSearch={flyToCoords} />
      </div>
    </>
  );
};