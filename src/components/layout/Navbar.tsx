...
const InputField: React.FC<{
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: "block", fontSize: 11, fontWeight: 600, fontFamily: FONT, letterSpacing: "0.06em", textTransform: "uppercase" as const, color: "#6b6b6b", marginBottom: 5 }}>
        {label}
      </label>
      <div style={{ position: "relative" }}>
        <input
          ...
          style={{ width: "100%", padding: "10px 14px", borderRadius: 8, border: `1.5px solid ${error ? "#fca5a5" : "#e4ddd4"}`, fontSize: 13, fontFamily: FONT, background: error ? "#fef2f2" : "#faf8f5", outline: "none", boxSizing: "border-box" as const, color: "#1a1a1a", transition: "border-color 0.18s, box-shadow 0.18s" }}
          onFocus={(e) => { e.currentTarget.style.borderColor = error ? "#f87171" : BLUE_DEEP; e.currentTarget.style.boxShadow = error ? "none" : "0 0 0 3px rgba(26,59,191,0.08)"; e.currentTarget.style.background = "#fff"; }}
          onBlur={(e)  => { e.currentTarget.style.borderColor = error ? "#fca5a5" : "#e4ddd4"; e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.background = error ? "#fef2f2" : "#faf8f5"; }}
        />
        {isPwd && (
          <button type="button" onClick={() => setShowPass(p => !p)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#9a9a9a" }}>
...
const GoogleButton: React.FC<{ label: string; onClick: () => void; loading?: boolean }> = ({ label, onClick, loading }) => (
  <button onClick={onClick} disabled={loading}
    style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "10px 16px", borderRadius: 8, border: "1.5px solid #e4ddd4", background: "#fff", cursor: loading ? "not-allowed" : "pointer", fontFamily: FONT, fontSize: 13, fontWeight: 600, color: "#2b2b2b", opacity: loading ? 0.7 : 1, transition: "all 0.15s" }}
...
const RoleStep: React.FC<{ selectedRole: UserRole; onSelect: (r: UserRole) => void; language: string }> = ({ selectedRole, onSelect, language }) => (
  <div>
    <p style={{ fontFamily: FONT, fontSize: 13, color: "#6b6b6b", marginBottom: 14, lineHeight: 1.6 }}>
      {language === "id" ? "Pilih jenis akun Anda. Anda dapat mengubahnya nanti di profil." : "Choose your account type. You can change this later in your profile."}
    </p>
      ...
      {ROLE_OPTS.map((r) => {
        ...
        return (
          <button key={r.value} onClick={() => onSelect(r.value)}
            style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", borderRadius: 10, width: "100%", textAlign: "left", border: sel ? `2px solid ${BLUE_DEEP}` : "1.5px solid #e4ddd4", background: sel ? "#eef1fb" : "#fff", cursor: "pointer", transition: "all 0.15s" }}
            ...
            <div style={{ width: 34, height: 34, borderRadius: 8, flexShrink: 0, background: sel ? BLUE_DEEP : "#f2ede6", display: "flex", alignItems: "center", justifyContent: "center", color: sel ? "#fff" : "#6b6b6b", transition: "all 0.15s" }}>
              {r.icon}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontFamily: FONT, fontSize: 13, fontWeight: 600, color: sel ? BLUE_DEEP : "#1a1a1a", margin: "0 0 2px" }}>{language === "id" ? r.labelId : r.labelEn}</p>
              <p style={{ fontFamily: FONT, fontSize: 11, color: sel ? "#4a68d0" : "#9a9a9a", margin: 0, lineHeight: 1.5 }}>{language === "id" ? r.descId : r.descEn}</p>
            </div>
            <div style={{ width: 16, height: 16, borderRadius: "50%", flexShrink: 0, marginTop: 2, border: `2px solid ${sel ? BLUE_DEEP : "#ccc8c0"}`, background: sel ? BLUE_DEEP : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
...
const AuthModal: React.FC<{
  ...
  const primaryBtn: React.CSSProperties = { width: "100%", padding: "11px", borderRadius: 8, border: "none", cursor: "pointer", fontFamily: FONT, fontSize: 13, fontWeight: 600, background: BLUE_DEEP, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background 0.2s, transform 0.15s" };

  const StepDots = () => (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 16 }}>
      {([1, 2] as SignupStep[]).map((s, i) => (
        <React.Fragment key={s}>
          <div style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: suStep >= s ? BLUE_DEEP : "#e4ddd4", fontSize: 10, fontWeight: 700, fontFamily: FONT, color: suStep >= s ? "#fff" : "#9a9a9a", transition: "all 0.2s" }}>
  ...
  return (
    <div ref={overlayRef} onClick={(e) => { if (e.target === overlayRef.current) onClose(); }}
      style={{ position: "fixed", inset: 0, background: "rgba(26,26,26,0.45)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 14, padding: "28px 24px 24px", width: "100%", maxWidth: 400, position: "relative", boxShadow: "0 24px 60px rgba(0,0,0,0.16)", maxHeight: "90vh", overflowY: "auto", border: "1px solid rgba(0,0,0,0.06)" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 14, right: 14, background: "none", border: "none", cursor: "pointer", color: "#9a9a9a", padding: 6, borderRadius: 6 }} onMouseEnter={(e) => (e.currentTarget.style.background = "#f2ede6")} onMouseLeave={(e) => (e.currentTarget.style.background = "none")}><X size={14} /></button>
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <h2 style={{ fontFamily: FONT, fontSize: 17, fontWeight: 700, color: "#1a1a1a", margin: 0 }}>
              {isSignIn ? (language === "id" ? "Masuk ke Searibu" : "Sign In to Searibu") : suStep === 1 ? (language === "id" ? "Buat Akun Baru" : "Create Account") : (language === "id" ? "Jenis Akun" : "Account Type")}
            </h2>
          </div>
          <p style={{ fontFamily: FONT, fontSize: 12, color: "#9a9a9a", marginLeft: 40 }}>
            {isSignIn ? (language === "id" ? "Selamat datang kembali" : "Welcome back") : (language === "id" ? "Bergabunglah dengan Searibu" : "Join the Searibu community")}
          </p>
        </div>
        ...
        {isSignIn && (<>
          ...
          <button onClick={doSignIn} disabled={siLoad} style={{ ...primaryBtn, opacity: siLoad ? 0.65 : 1, marginTop: 4 }} onMouseEnter={(e) => { e.currentTarget.style.background = "#142d99"; e.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={(e) => { e.currentTarget.style.background = BLUE_DEEP; e.currentTarget.style.transform = "none"; }}>
          ...
          <p style={{ textAlign: "center", marginTop: 14, fontSize: 12, color: "#6b6b6b", fontFamily: FONT }}>
            ...
            <button onClick={() => onSwitch("signup")} style={{ background: "none", border: "none", cursor: "pointer", color: BLUE_DEEP, fontWeight: 600, fontSize: 12, fontFamily: FONT, textDecoration: "underline" }}>{language === "id" ? "Daftar" : "Sign up"}</button>
          </p>
        </>)}
        ...
        {!isSignIn && suStep === 1 && (<>
          ...
          <button onClick={doStep1} style={{ ...primaryBtn, marginTop: 4 }} onMouseEnter={(e) => { e.currentTarget.style.background = "#142d99"; e.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={(e) => { e.currentTarget.style.background = BLUE_DEEP; e.currentTarget.style.transform = "none"; }}>
            {language === "id" ? "Lanjut" : "Continue"}
          </button>
          <p style={{ textAlign: "center", marginTop: 14, fontSize: 12, color: "#6b6b6b", fontFamily: FONT }}>
            {language === "id" ? "Sudah punya akun?" : "Already have an account?"}{" "}
            <button onClick={() => onSwitch("signin")} style={{ background: "none", border: "none", cursor: "pointer", color: BLUE_DEEP, fontWeight: 600, fontSize: 12, fontFamily: FONT, textDecoration: "underline" }}>{language === "id" ? "Masuk" : "Sign in"}</button>
          </p>
        </>)}
        ...
        {!isSignIn && suStep === 2 && (<>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button onClick={() => setSuStep(1)} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "1.5px solid #e4ddd4", background: "#fff", fontFamily: FONT, fontSize: 13, fontWeight: 600, color: "#6b6b6b", cursor: "pointer" }}>{language === "id" ? "Kembali" : "Back"}</button>
            <button onClick={doSignUp} disabled={suLoad} style={{ flex: 2, ...primaryBtn, opacity: suLoad ? 0.65 : 1 }} onMouseEnter={(e) => { e.currentTarget.style.background = "#142d99"; }} onMouseLeave={(e) => { e.currentTarget.style.background = BLUE_DEEP; }}>
  ...
  return (
    <>
      <button ref={btnRef} onClick={openDrop}
        style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 10px 5px 6px", borderRadius: 8, border: lightMode ? "1px solid #e4ddd4" : "none", background: lightMode ? "#f7f4ef" : "transparent", cursor: "pointer", transition: "background 0.15s" }}
        ...
        {user.avatar
          ...
          : <div style={{ width: 28, height: 28, borderRadius: "50%", background: AMBER, display: "flex", alignItems: "center", justifyContent: "center", color: DARK1, fontSize: 11, fontWeight: 700, fontFamily: FONT, flexShrink: 0 }}>{initials}</div>}
...
const MobileSubBadge: React.FC<{ language: string }> = ({ language }) => {
  ...
  if (isAdmin) return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 11px", borderRadius: 99, border: "1px solid rgba(245,193,24,0.50)", background: "rgba(245,193,24,0.10)", fontFamily: FONT, fontSize: 12, fontWeight: 700, color: "#b8940a", userSelect: "none" }}>
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 11px", borderRadius: 99, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700, transition: "all 0.15s", ...(isPro ? isProStyle : freeStyle) }} onMouseEnter={(e) => (e.currentTarget.style.opacity = "0.80")} onMouseLeave={(e) => (e.currentTarget.style.opacity = "1")}>
        <Zap size={11} fill={isPro ? "#15803d" : "none"} />{label}
      </button>
...
const DesktopSubBadge: React.FC<{ language: string }> = ({ language }) => {
  ...
  if (isAdmin) return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 99, border: "1px solid rgba(245,193,24,0.45)", background: "rgba(245,193,24,0.12)", fontFamily: FONT, fontSize: 11, fontWeight: 700, color: AMBER, userSelect: "none", cursor: "default", letterSpacing: "0.03em" }} title="Admin — full access">
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} title={isPro ? "Pro plan active" : "Upgrade to Pro"}
        style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 99, border: isPro ? "1px solid rgba(157,224,90,0.35)" : "1px solid rgba(255,255,255,0.20)", background: isPro ? "rgba(157,224,90,0.12)" : "rgba(255,255,255,0.07)", cursor: "pointer", fontFamily: FONT, fontSize: 11, fontWeight: 700, color: isPro ? GREEN : "rgba(245,240,232,0.60)", transition: "all 0.18s", letterSpacing: "0.03em" }}
    ...
    <>
        ...
        <div style={{ maxWidth: 1360, margin: "0 auto", height: "100%", display: "flex", alignItems: "center", padding: "0 16px", gap: 0 }}>
          ...
          <nav className="nb-desktop-nav" style={{ display: "flex", alignItems: "center", gap: 28, flex: 1 }}>
            {navLabels.map((label, i) => (
              <button key={NAV_KEYS[i]} onClick={() => goTo(NAV_KEYS[i])} className={`nb-link ${activePage === NAV_KEYS[i] ? "active" : ""}`}>{label}</button>
            ))}
          </nav>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 1, background: "rgba(255,255,255,0.08)", borderRadius: 8, padding: "3px", flexShrink: 0 }}>
              {(["en", "id"] as const).map(l => (
                <button key={l} onClick={() => setLanguage(l)} style={{ fontFamily: FONT, fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", padding: "4px 9px", borderRadius: 5, border: "none", cursor: "pointer", transition: "all 0.15s", background: language === l ? AMBER : "transparent", color: language === l ? DARK1 : "rgba(245,240,232,0.45)" }}>
                  {l.toUpperCase()}
                </button>
              ))}
            </div>
      ...
      {mobileOpen && (
        <div className="nb-mobile-drawer">
          {navLabels.map((label, i) => (
            <button key={NAV_KEYS[i]} onClick={() => goTo(NAV_KEYS[i])} className={`nb-mobile-link ${activePage === NAV_KEYS[i] ? "active" : ""}`}>{label}</button>
          ))}
