  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{
        ...
        letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#64748b', marginBottom: 6
      }}>
        {label}
      </label>
      <div style={{ position: 'relative' }}>
        <input
          ...
          style={{
            ...
            fontSize: 14, fontFamily: SANS, background: error ? '#fef2f2' : '#f8fafc',
            outline: 'none', boxSizing: 'border-box' as const, color: '#0f172a',
            transition: 'border-color 0.2s, box-shadow 0.2s',
          }}
          onFocus={e => { e.currentTarget.style.borderColor = error ? '#f87171' : PRIMARY; e.currentTarget.style.boxShadow = `0 0 0 3px rgba(3,105,161,0.10)`; e.currentTarget.style.background = '#fff'; }}
          onBlur={e => { e.currentTarget.style.borderColor = error ? '#fca5a5' : '#e2e8f0'; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.background = error ? '#fef2f2' : '#f8fafc'; }}
        />
        {isPassword && (
...
const GoogleButton: React.FC<{ label: string; onClick: () => void; loading?: boolean }> = ({ label, onClick, loading }) => (
  <button
    ...
    onMouseEnter={e => { if (!loading) { e.currentTarget.style.borderColor = '#d1d5db'; e.currentTarget.style.background = '#f8fafc'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,0.08)'; } }}
...
const AuthModal: React.FC<{
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (backendRes.ok) {
          ...
          onSuccess({ ...data.user, avatar: profile.picture }, data.message || (language === 'id' ? 'Berhasil masuk dengan Google!' : 'Signed in with Google!'));
        } else {
          const syntheticUser: AuthUser = { id: 0, full_name: profile.name, email: profile.email, created_at: new Date().toISOString(), avatar: profile.picture };
          onSuccess(syntheticUser, language === 'id' ? 'Berhasil masuk dengan Google!' : 'Signed in with Google!');
        }
      } catch (e: any) {
  ...
  return (
    <div ref={overlayRef} onClick={e => { if (e.target === overlayRef.current) onClose(); }}
      style={{
        position: 'fixed', inset: 0, background: 'rgba(2,78,120,0.35)', backdropFilter: 'blur(6px)',
        display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200,
        animation: 'mfIn 0.18s ease', WebkitBackdropFilter: 'blur(6px)',
      }}>
      <div style={{
        background: '#fff', borderRadius: 14, padding: '32px 32px 28px', width: 400,
        maxWidth: '92vw', position: 'relative', boxShadow: '0 20px 60px rgba(2,78,120,0.18)',
        animation: 'mfScale 0.22s cubic-bezier(0.16,1,0.3,1)', maxHeight: '92vh', overflowY: 'auto',
      }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <h2 style={{ fontFamily: SANS, fontSize: 18, fontWeight: 800, color: '#0f172a', margin: 0, letterSpacing: '-0.02em' }}>
              {isSignIn ? (language === 'id' ? 'Masuk ke Searibu' : 'Sign In to Searibu') : (language === 'id' ? 'Buat Akun Baru' : 'Create Account')}
            </h2>
          </div>
          <p style={{ fontFamily: SANS, fontSize: 13, color: '#94a3b8', marginLeft: 40 }}>
            {isSignIn ? (language === 'id' ? 'Selamat datang kembali' : 'Welcome back') : (language === 'id' ? 'Bergabunglah dengan komunitas Searibu' : 'Join the Searibu community')}
          </p>
        </div>
        ...
        {isSignIn ? (
          <>
            ...
            <button onClick={doSignIn} disabled={siLoad} style={{ ...primaryBtn, opacity: siLoad ? 0.65 : 1 }}
          ...
          <>
            ...
            <button onClick={doSignUp} disabled={suLoad} style={{ ...primaryBtn, opacity: suLoad ? 0.65 : 1 }}
              onMouseEnter={e => (e.currentTarget.style.background = NAVY)}
              onMouseLeave={e => (e.currentTarget.style.background = PRIMARY)}>
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)}
        ...
        {user.avatar ? (
          <img src={user.avatar} alt={user.full_name} style={{ width: 30, height: 30, borderRadius: '50%', objectFit: 'cover', flexShrink: 0, border: '2px solid rgba(14,165,233,0.4)' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
        ) : (
...
const SuccessToast: React.FC<{ message: string; userName: string; onClose: () => void }> = ({ message, userName, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 76, left: '50%', transform: 'translateX(-50%)', zIndex: 300, animation: 'toastIn 0.38s cubic-bezier(0.16,1,0.3,1)', pointerEvents: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1px solid #bbf7d0', borderRadius: 12, padding: '12px 20px', boxShadow: '0 8px 32px rgba(22,163,74,0.14)', minWidth: 300, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontFamily: SANS, fontSize: 12, color: '#94a3b8' }}>
            Halo, <span style={{ color: PRIMARY, fontWeight: 600 }}>{userName}</span>!
          </p>
        </div>
    ...
    <>
      ...
      <style>{`
        ...
        .nb-mobile-link.active {
          color: ${PRIMARY}; border-left-color: ${PRIMARY_SOFT};
          background: rgba(3,105,161,0.06); font-weight: 600;
        }
      ...
      <header style={{
        ...
        <div style={{
          ...
          >
            <img src="/logo.svg" alt="Searibu" className="nb-logo-img"
              style={{ height: 36, width: 'auto', display: 'block', filter: 'brightness(0) invert(1)', transition: 'filter 0.32s ease' }}
            />
          </button>
          ...
          <nav className="nb-desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: 28, flex: 1 }}>
            {navItems.map((label, i) => (
              <button
                ...
                style={{
                  color: activePage === NAV_KEYS[i] ? '#fff' : 'rgba(255,255,255,0.65)',
                }}
              >
                {label}
              </button>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: 7, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
      ...
      {mobileOpen && (
        <div className="nb-mobile-drawer">
          {navItems.map((label, i) => (
            <button key={NAV_KEYS[i]} onClick={() => setActivePage(NAV_KEYS[i])} className={`nb-mobile-link ${activePage === NAV_KEYS[i] ? 'active' : ''}`}>
              {label}
            </button>
          ))}
        </div>
