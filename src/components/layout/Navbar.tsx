  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{
        ...
        letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: '#6b7280', marginBottom: 6
      }}>
        {label}
      </label>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: '#c2410c', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Anchor size={14} color="#fff" />
            </div>
            <h2 style={{ fontFamily: SANS, fontSize: 18, fontWeight: 800, color: '#111827', margin: 0, letterSpacing: '-0.02em' }}>
              {isSignIn
...
const SuccessToast: React.FC<{ message: string; userName: string; onClose: () => void }> = ({ message, userName, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 76, left: '50%', transform: 'translateX(-50%)', zIndex: 300, animation: 'toastIn 0.38s cubic-bezier(0.16,1,0.3,1)', pointerEvents: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: '#fff', border: '1px solid #d1fae5', borderRadius: 12, padding: '12px 20px', boxShadow: '0 8px 32px rgba(22,163,74,0.14)', minWidth: 300, position: 'relative', overflow: 'hidden' }}>
  ...
  return (
    <>
      ...
      <header style={{
        ...
        background: glass
          ? 'linear-gradient(to bottom, rgba(7,18,28,0.72) 0%, rgba(7,18,28,0) 100%)'
          : 'rgba(255,255,255,0.97)',
        backdropFilter: scrolled ? 'blur(18px)' : 'none',
        WebkitBackdropFilter: scrolled ? 'blur(18px)' : 'none',
        borderBottom: glass ? 'none' : '1px solid rgba(0,0,0,0.07)',
        boxShadow: scrolled && !glass ? '0 1px 12px rgba(0,0,0,0.06)' : 'none',
        ...
        <div style={{
          ...
          <nav className="nb-desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: 28, flex: 1 }}>
            {navItems.map((label, i) => (
              <button
                ...
                style={{
                  color: glass
                    ? (activePage === NAV_KEYS[i] ? '#fff' : 'rgba(255,255,255,0.65)')
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {currentUser ? (
              <UserDropdown
                user={currentUser}
                onLogout={() => { setCurrentUser(null); sessionStorage.removeItem('searibu_user'); }}
                language={language}
              />
            ) : (
              <button
                ...
                className="nb-signin"
                style={{
                  borderColor: glass ? 'rgba(255,255,255,0.42)' : '#e8600a',
                ...
                onMouseLeave={e => { const b = e.currentTarget; b.style.background = glass ? 'rgba(255,255,255,0.08)' : 'transparent'; b.style.borderColor = glass ? 'rgba(255,255,255,0.42)' : '#e8600a'; b.style.color = glass ? 'rgba(255,255,255,0.85)' : '#c2410c'; }}
              >
                <LogIn size={13} />
                {language === 'id' ? 'Masuk' : 'Sign In'}
              </button>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: 7, color: glass ? '#fff' : '#374151', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
