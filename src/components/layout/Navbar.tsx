...
const BLUE_MID     = "#4fd4e8";
const BLUE_LIGHT   = "#ddf0fb";
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{
        ...
        textTransform: "uppercase" as const, color: "#6b6b6b", marginBottom: 5,
      }}>
        {label}
      </label>
      <div style={{ position: "relative" }}>
        <input
          ...
          style={{
            ...
            borderRadius: 8, border: `1.5px solid ${error ? "#fca5a5" : "#e4ddd4"}`,
            fontSize: 13, fontFamily: FONT,
            background: error ? "#fef2f2" : "#faf8f5",
            outline: "none", boxSizing: "border-box" as const,
            color: "#1a1a1a", transition: "border-color 0.18s, box-shadow 0.18s",
          }}
          onFocus={(e) => {
            e.currentTarget.style.borderColor = error ? "#f87171" : BLUE_DEEP;
            e.currentTarget.style.boxShadow = error ? "none" : `0 0 0 3px rgba(26,59,191,0.08)`;
            e.currentTarget.style.background = "#fff";
          }}
          onBlur={(e) => {
            e.currentTarget.style.borderColor = error ? "#fca5a5" : "#e4ddd4";
        ...
        {isPassword && (
          <button type="button" onClick={() => setShowPass((p) => !p)}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#9a9a9a" }}>
...
const OrDivider: React.FC<{ language: string }> = ({ language }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "14px 0" }}>
    ...
    <span style={{ fontFamily: FONT, fontSize: 11, color: "#9a9a9a" }}>
      {language === "id" ? "atau" : "or"}
    </span>
    <div style={{ flex: 1, height: 1, background: "#e4ddd4" }} />
  </div>
...
const RoleStep: React.FC<{
  selectedRole: UserRole; onSelect: (r: UserRole) => void; language: string;
        ...
        return (
          <button
            ...
            style={{
              ...
              border: isSelected ? `2px solid ${BLUE_DEEP}` : "1.5px solid #e4ddd4",
              background: isSelected ? "#eef1fb" : "#fff",
              cursor: "pointer", transition: "all 0.15s",
            }}
            onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.borderColor = "#a0adf5"; }}
            onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.borderColor = "#e4ddd4"; }}
          >
            <div style={{
              width: 34, height: 34, borderRadius: 8, flexShrink: 0,
              background: isSelected ? BLUE_DEEP : "#f2ede6",
              display: "flex", alignItems: "center", justifyContent: "center",
              color: isSelected ? "#fff" : "#6b6b6b", transition: "all 0.15s",
            }}>
              {r.icon}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontFamily: FONT, fontSize: 13, fontWeight: 600, color: isSelected ? BLUE_DEEP : "#1a1a1a", margin: "0 0 2px" }}>
                {language === "id" ? r.labelId : r.labelEn}
              </p>
              <p style={{ fontFamily: FONT, fontSize: 11, color: isSelected ? "#4a68d0" : "#9a9a9a", margin: 0, lineHeight: 1.5 }}>
                {language === "id" ? r.descId : r.descEn}
              </p>
            </div>
            <div style={{
              width: 16, height: 16, borderRadius: "50%", flexShrink: 0, marginTop: 2,
              border: `2px solid ${isSelected ? BLUE_DEEP : "#ccc8c0"}`,
              background: isSelected ? BLUE_DEEP : "transparent",
              display: "flex", alignItems: "center", justifyContent: "center",
...
const AuthModal: React.FC<{
  ...
  const [suStep,  setSuStep]  = useState<SignupStep>(1); const [suRole, setSuRole] = useState<UserRole>("general");
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (backendRes.ok) {
          ...
          onSuccess({ id: 0, full_name: profile.name, email: profile.email, role: "general", created_at: new Date().toISOString(), avatar: profile.picture, is_admin: false }, language === "id" ? "Berhasil masuk!" : "Signed in!");
        }
      } catch (e: any) { const msg = e.message || "Google sign-in failed"; setSiErr(msg); setSuApi(msg); }
  ...
  const primaryBtn: React.CSSProperties = {
    ...
    background: BLUE_DEEP, color: "#fff",
      ...
      {([1, 2] as SignupStep[]).map((s, i) => (
        <React.Fragment key={s}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            background: suStep >= s ? BLUE_DEEP : "#e4ddd4",
  ...
  return (
    ...
    >
      <div style={{
        background: "#fff", borderRadius: 14, padding: "28px 24px 24px",
        ...
        <button onClick={onClose}
          style={{ position: "absolute", top: 14, right: 14, background: "none", border: "none", cursor: "pointer", color: "#9a9a9a", padding: 6, borderRadius: 6 }}
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: BLUE_DEEP, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        ...
        {isSignIn && (
          <>
            ...
            {siErr && (
              <div style={{ marginBottom: 12, padding: "9px 12px", borderRadius: 8, background: "#fef2f2", border: "1px solid #fca5a5" }}>
                <p style={{ fontSize: 12, color: "#dc2626", fontFamily: FONT, margin: 0 }}>{siErr}</p>
              </div>
            )}
            <button onClick={doSignIn} disabled={siLoad}
              style={{ ...primaryBtn, opacity: siLoad ? 0.65 : 1, marginTop: 4 }}
            ...
            <p style={{ textAlign: "center", marginTop: 14, fontSize: 12, color: "#6b6b6b", fontFamily: FONT }}>
              ...
              <button onClick={() => onSwitch("signup")}
                style={{ background: "none", border: "none", cursor: "pointer", color: BLUE_DEEP, fontWeight: 600, fontSize: 12, fontFamily: FONT, textDecoration: "underline" }}>
        ...
        {!isSignIn && suStep === 1 && (
          <>
            ...
            {suApi && (
              <div style={{ marginBottom: 12, padding: "9px 12px", borderRadius: 8, background: "#fef2f2", border: "1px solid #fca5a5" }}>
                <p style={{ fontSize: 12, color: "#dc2626", fontFamily: FONT, margin: 0 }}>{suApi}</p>
              </div>
            )}
        ...
        {!isSignIn && suStep === 2 && (
          <>
            ...
            {suApi && (
              <div style={{ marginTop: 12, padding: "9px 12px", borderRadius: 8, background: "#fef2f2", border: "1px solid #fca5a5" }}>
                <p style={{ fontSize: 12, color: "#dc2626", fontFamily: FONT, margin: 0 }}>{suApi}</p>
              </div>
            )}
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button onClick={() => setSuStep(1)}
                style={{ flex: 1, padding: "10px", borderRadius: 8, border: "1.5px solid #e4ddd4", background: "#fff", fontFamily: FONT, fontSize: 13, fontWeight: 600, color: "#6b6b6b", cursor: "pointer" }}>
                {language === "id" ? "Kembali" : "Back"}
              </button>
              <button onClick={doSignUp} disabled={suLoad}
                style={{ flex: 2, ...primaryBtn, opacity: suLoad ? 0.65 : 1 }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "#142d99"; }}
      ...
      <div ref={ref} style={{ position: "relative" }}>
        <button
          ...
          style={{
            ...
            padding: "5px 10px 5px 6px", borderRadius: 8, border: "none",
        ...
        >
          {user.avatar
            ? <img src={user.avatar} alt={user.full_name} style={{ width: 30, height: 30, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "2px solid rgba(245,193,24,0.5)" }} />
            : (
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: AMBER, display: "flex", alignItems: "center", justifyContent: "center", color: DARK1, fontSize: 11, fontWeight: 700, fontFamily: FONT, flexShrink: 0 }}>
                {initials}
              </div>
            )
          }
          <span className="nb-username" style={{ fontFamily: FONT, fontSize: 13, fontWeight: 600, color: OFF_WHITE, maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {user.full_name.split(" ")[0]}
          </span>
          <ChevronDown size={12} style={{ color: "rgba(245,240,232,0.55)", transform: open ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
        </button>

        {open && (
          <div style={{
            position: "absolute", right: 0, top: 46, width: 228,
            background: "#fff", border: "1px solid #e4ddd4", borderRadius: 12,
            boxShadow: "0 16px 40px rgba(0,0,0,0.14), 0 4px 12px rgba(0,0,0,0.06)",
            ...
            <div style={{ padding: "12px 14px", borderBottom: "1px solid #f2ede6" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                {user.avatar
                  ...
                  : <div style={{ width: 34, height: 34, borderRadius: "50%", background: AMBER, display: "flex", alignItems: "center", justifyContent: "center", color: DARK1, fontSize: 12, fontWeight: 700, fontFamily: FONT, flexShrink: 0 }}>{initials}</div>
                }
                <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {isAdmin && (
              <button onClick={() => { setOpen(false); setAdminOpen(true); }}
                ...
                onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(245,193,24,0.07)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "none")}>
                <ShieldCheck size={13} />
...
const SuccessToast: React.FC<{ message: string; userName: string; onClose: () => void }> = ({ message, userName, onClose }) => {
  ...
  return (
    <div style={{ position: "fixed", top: 82, left: "50%", transform: "translateX(-50%)", zIndex: 300, pointerEvents: "none", width: "calc(100% - 32px)", maxWidth: 360 }}>
    ...
    <>
      ...
      <style>{`
        ...
        .nb-mobile-link.active {
          color: ${BLUE_DEEP};
          border-left-color: ${BLUE_DEEP};
          background: rgba(26,59,191,0.05);
          font-weight: 600;
        }
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1360, margin: "0 auto", height: "100%", display: "flex", alignItems: "center", padding: "0 18px", gap: 0 }}>
          {/* Logo */}
          <button
            onClick={() => setActivePage("home")}
            style={{ display: "flex", alignItems: "center", background: "none", border: "none", cursor: "pointer", padding: "4px 0", marginRight: 36, flexShrink: 0 }}
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            {/* Language toggle */}
            <div style={{ display: "flex", alignItems: "center", gap: 1, background: "rgba(255,255,255,0.08)", borderRadius: 8, padding: "3px" }}>
